...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLocalizedText() {
      ...
      if (appName)
        appName.textContent = extensionApi.i18n.getMessage('appName');
      if (appSubtitle)
        appSubtitle.textContent = extensionApi.i18n.getMessage('appSubtitle');
      if (goToProfile)
        goToProfile.textContent = extensionApi.i18n.getMessage('goToProfile');
      if (infoText)
        infoText.textContent = extensionApi.i18n.getMessage('infoText');
      if (issuesFeedback)
        issuesFeedback.textContent =
          extensionApi.i18n.getMessage('issuesFeedback');
    }
    ...
    githubBtn?.addEventListener('click', () => {
      extensionApi.tabs.create({
        url: 'https://github.com/enkoding1/42-time-extension',
      });
    });
  });
})();
